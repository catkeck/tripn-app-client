...
export function fetchStringWeather(location) {
  return function(dispatch) {
    ...
    fetch(`https://api.openweathermap.org/data/2.5/weather?q=${location}&units=imperial&APPID=600c24f64be0542ee3eea23ad03ee915`)
...
export function fetchCoordinateWeather(location) {
  ...
  return function(dispatch) {
    dispatch({type: "FETCHING_WEATHER"})
    fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${latlon[0]}&lon=${latlon[1]}&units=imperial&APPID=600c24f64be0542ee3eea23ad03ee915`)
      .then(response => response.json())
      .then(weather => {
        dispatch({type: "FETCHED_WEATHER", payload: weather
        })
